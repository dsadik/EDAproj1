source('~/EDAproj1/ReadSubsetData.R')
subsetdata$Global_active_power <- as.numeric(as.character(subsetdata$Global_active_power))
subsetdata$Global_reactive_power <- as.numeric(as.character(subsetdata$Global_reactive_power))
subsetdata$Voltage <- as.numeric(as.character(subsetdata$Voltage))
subsetdata$Global_intensity <- as.numeric(as.character(subsetdata$Global_intensity))
subsetdata$dt <- paste(subsetdata$Date, subsetdata$Time)
subsetdata$dt <- strptime(subsetdata$dt, "%d/%m/%Y %H:%M:%S")
subsetdata$Sub_metering_1 <- as.numeric(as.character(subsetdata$Sub_metering_1))
subsetdata$Sub_metering_2 <- as.numeric(as.character(subsetdata$Sub_metering_2))
subsetdata$Sub_metering_3 <- as.numeric(as.character(subsetdata$Sub_metering_3))
png(file = "plot4.png", width = 480, height = 480)
par(mfrow=c(2,2))
plot(subsetdata$dt,subsetdata$Global_active_power, type="l", xlab="", ylab="Global Active Power")
plot(subsetdata$dt,subsetdata$Voltage, type="l", xlab="datetime", ylab="Voltage")
plot(subsetdata$dt,subsetdata$Sub_metering_1, type="l", xlab="", ylab="Energy sub metering")
lines(subsetdata$dt, subsetdata$Sub_metering_2, type="l", col="red")
lines(subsetdata$dt, subsetdata$Sub_metering_3, type="l", col="blue")
legend("topright", bty="n", lty=1, col=c("black", "red", "blue"), legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
plot(subsetdata$dt,subsetdata$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")
dev.off()